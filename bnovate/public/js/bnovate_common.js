...
/************** Certificate migration utils ***************************/


// Run this on the "Enclosure filling history" report
bnovate.utils.migrate_certificates3 = async function () {

  const data = frappe.query_report.data
    .filter(row => !row.analysis_certificate);


  for (let [i, row] of data.entries()) {
    frappe.show_progress("Working...", i, data.length, `Fixing certificate for ${row.fill_serial}. ${i}/${data.length}`);
    ...
    if (!sn_doc.delivery_document_no) continue;
